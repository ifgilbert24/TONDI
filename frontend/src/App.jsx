...
import { BarChart3, MapPin, AlertTriangle, Star, TrendingUp, Clock, ChevronDown, MessageSquare, CheckCircle2 } from 'lucide-react';
...
const API_BASE = 'https://thumbzilla-vision-conservative-infant.trycloudflare.com/api';
  ...
  return (
    <div className="bg-gradient-to-br from-slate-50 to-white p-3.5 rounded-xl border border-slate-200/70 shadow-sm hover:shadow-md hover:border-slate-300/70 transition-all duration-200">
        ...
        <div className="flex items-center gap-2">
          {review.rating && (
            <div className="flex items-center gap-0.5 bg-amber-50/80 px-1.5 py-0.5 rounded-md">
              {[...Array(5)].map((_, i) => (
                <Star key={i} size={9} className={i < (review.rating || 3) ? "text-amber-400 fill-amber-400" : "text-amber-200/50"} />
              ))}
            </div>
          )}
          {review.date && <span className="text-[10px] text-slate-400 tabular-nums">{review.date?.slice(0, 10) || '-'}</span>}
        </div>
      </div>
      ...
      <div className="flex gap-2 mt-2.5">
        <span className={'text-[10px] font-semibold px-2.5 py-0.5 rounded-full ring-1 '+(review.sentiment==='Positif'?'bg-emerald-50 text-emerald-700 ring-emerald-200/50':review.sentiment==='Negatif'?'bg-rose-50 text-rose-700 ring-rose-200/50':'bg-slate-50 text-slate-600 ring-slate-200/50')}>
          {review.sentiment}
        </span>
        {review.topic && (
...
export default function App() {
  ...
  useEffect(() => {
    fetch('/data/location-images.json').then(r => r.ok ? r.json() : null).then(data => {
      if (data) {
        if (data.images) setExcelImages(data.images);
        if (data.normalized) setExcelNormalized(data.normalized);
      fetch(API_BASE + '/dashboard', { headers: { 'ngrok-skip-browser-warning': 'true' } })
    }
    }).catch(() => {});
  }, []);
  ...
  function getCoord(name) {
    const found = locations.find(l => l.name === name && l.latitude && l.longitude);
  ...
  async function getWikiThumb(name) {
    try {
      const url = 'https://id.wikipedia.org/w/api.php?action=query&prop=pageimages&piprop=thumbnail&pithumbsize=400&titles='
        + encodeURIComponent(name) + '&format=json&origin=*';
  ...
  async function clickLoc(name) {
    ...
    if (flyToCoord && mapRef.current) {
      mapRef.current.flyTo([flyToCoord.lat, flyToCoord.lng], 13, { duration: 1.2 });
    }
    try {
      const [locRes, recRes] = await Promise.all([
        fetch(API_BASE+'/locations/'+encodeURIComponent(name)).then(r=>r.ok?r.json():null),
        fetch(API_BASE+'/recommendations/'+encodeURIComponent(name)).then(r=>r.ok?r.json():null),
      ]);
      if (locRes) {
        setLocDetail(locRes);
        // Fallback: use coordinate from detail endpoint
        if (!flyToCoord && locRes.coordinate && locRes.coordinate.lat && mapRef.current) {
          mapRef.current.flyTo([locRes.coordinate.lat, locRes.coordinate.lng], 13, { duration: 1.2 });
        }
      }
      ...
      <div className="min-h-screen bg-slate-50 text-slate-800 font-sans p-4 xl:p-6 animate-fadeIn">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="flex flex-col gap-6">
            ...
            {/* Map skeleton */}
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm" style={{ minHeight: '340px' }}>
              <Skeleton className="h-4 w-36 mb-3" />
              <div className="rounded-lg bg-slate-100" style={{ height: '280px' }}>
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-xl border border-red-200 shadow-lg max-w-md text-center">
          ...
          <button onClick={() => window.location.reload()} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">Muat Ulang</button>
        </div>
      </div>
    );
  }
  ...
  const topicColors = ['#3b82f6','#ef4444','#f59e0b','#8b5cf6','#10b981','#6366f1','#ec4899','#14b8a6'];
  const pieData = locDetail?.topic_distribution
    ? Object.entries(locDetail.topic_distribution).map(([n,v],i)=>({name:n,value:v,color:topicColors[i%8]})).sort((a,b)=>b.value-a.value).slice(0,5)
    : dash?.topic_distribution
      ? Object.entries(dash.topic_distribution).map(([n,v],i)=>({name:n,value:v,color:topicColors[i%8]})).sort((a,b)=>b.value-a.value).slice(0,5)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100/80 text-slate-800 font-sans p-4 xl:p-6 dashboard-enter">
      ...
      <div className="mb-8 relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-900 via-blue-800 to-blue-950 p-6 md:p-8 shadow-lg">
        <div className="absolute inset-0 opacity-[0.03]" style={{backgroundImage:'radial-gradient(circle at 25% 50%, white 1px, transparent 1px)', backgroundSize:'24px 24px'}} />
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <h1 className="text-3xl md:text-4xl font-bold text-white tracking-tight">TONDI</h1>
              <span className="px-2.5 py-0.5 rounded-full bg-white/15 text-white/80 text-[10px] font-semibold uppercase tracking-wider border border-white/10">Dashboard</span>
            </div>
            <p className="text-blue-200/80 text-sm md:text-base font-light">Toba Observatory for Natural-language Destination Intelligence</p>
            <p className="text-blue-300/60 text-xs mt-1.5">Sistem Pemantauan Reputasi &amp; Integritas Destinasi Danau Toba</p>
          </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="group bg-white rounded-xl border border-slate-200/70 shadow-sm hover:shadow-md hover:border-blue-200/50 transition-all duration-300 overflow-hidden">
              ...
              <div className="p-4">
                <p className="text-slate-400 text-[11px] font-semibold uppercase tracking-wider mb-2">Total Lokasi</p>
                <div className="flex items-end justify-between">
                  <span className="text-2xl font-bold text-slate-900 tabular-nums tracking-tight">{kpiData.total_locations}</span>
            ...
            <div className="group bg-white rounded-xl border border-slate-200/70 shadow-sm hover:shadow-md hover:border-rose-200/50 transition-all duration-300 overflow-hidden">
              ...
              <div className="p-4">
                <p className="text-slate-400 text-[11px] font-semibold uppercase tracking-wider mb-2">Status Merah</p>
                <div className="flex items-end justify-between">
                  <span className="text-2xl font-bold text-rose-600 tabular-nums tracking-tight">{kpiData.red_status || 0}</span>
            ...
            <div className="group bg-white rounded-xl border border-slate-200/70 shadow-sm hover:shadow-md hover:border-amber-200/50 transition-all duration-300 overflow-hidden col-span-2">
              ...
              <div className="p-4 flex items-center justify-between">
                <div>
                  <p className="text-slate-400 text-[11px] font-semibold uppercase tracking-wider mb-2">Tren Keluhan Minggu Ini</p>
                  <div className="flex items-center gap-2.5">
                    <span className="text-2xl font-bold text-slate-900 tabular-nums tracking-tight">{kpiData.weekly_trend > 0 ? '+' : ''}{kpiData.weekly_trend || 0}%</span>
                    <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full ${kpiData.weekly_trend > 0 ? 'bg-rose-50 text-rose-600' : kpiData.weekly_trend < 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}>
                      {kpiData.weekly_trend > 0 ? <TrendingUp size={12} /> : kpiData.weekly_trend < 0 ? <TrendingUp size={12} className="rotate-180" /> : null}
                      {kpiData.weekly_trend > 0 ? 'Naik' : kpiData.weekly_trend < 0 ? 'Turun' : 'Stabil'}
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-400 mt-1.5">{kpiData.red_status || 0} lokasi merah, {anomalies.length} anomali</p>
                </div>
          ...
          <div className="bg-white/80 backdrop-blur-sm p-4 rounded-xl border border-slate-200/80 shadow-sm flex gap-3 sticky top-4 z-10">
            <div className="flex-1 relative">
              ...
              >
                ...
                {filters?.topics?.map(t => <option key={t}>{t}</option>)}
              </select>
          ...
          <div className="bg-white rounded-xl border border-slate-200/70 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden flex flex-col">
            <div className="px-4 pt-4 pb-3 flex items-center justify-between border-b border-slate-100">
              <h2 className="text-xs font-bold text-slate-600 uppercase tracking-wider flex items-center gap-2">
                <MapPin size={14} className="text-blue-500" /> Peta Persebaran Isu
              </h2>
            ...
            <div className="m-3 rounded-lg border border-slate-100 overflow-hidden z-0 relative" style={{ height: '340px', minHeight: '340px' }}>
              {mapMarkers.length > 0 && (
                <MapContainer center={[2.5839, 98.7816]} zoom={9} className="w-full h-full" scrollWheelZoom={true}>
                      ...
                      <Popup>
                        <div className="text-sm" style={{ minWidth: '200px' }}>
                          {getImageUrl(loc.name) && (
                            <img src={getImageUrl(loc.name)} alt={loc.name}
                          ...
                          <div className="flex items-center gap-2 mb-2">
                            <span style={{
                              ...
                              background: loc.status === 'red' ? '#fef2f2' : loc.status === 'yellow' ? '#fffbeb' : '#ecfdf5',
                              color: loc.status === 'red' ? '#dc2626' : loc.status === 'yellow' ? '#d97706' : '#059669',
                            }}>
                              {loc.status === 'red' ? 'Kritis' : loc.status === 'yellow' ? 'Waspada' : 'Aman'}
                            </span>
                            <span className="text-slate-400 text-xs">{loc.total_reviews} review</span>
                          </div>
                          <div className="flex gap-2 text-xs text-slate-500 mb-2">
                            <span>Rating: {loc.rating?.toFixed(1) || '-'}/5</span>
                            <span>|</span>
                            <span className="text-red-500">{loc.negatif} negatif</span>
                          </div>
                          {loc.region && <div className="text-xs text-slate-400 mb-2">Wilayah: {loc.region}</div>}
                          <button onClick={() => clickLoc(loc.name)}
                            style={{ width: '100%', padding: '5px 10px', background: '#2563eb', color: 'white', border: 'none', borderRadius: '6px', fontSize: '11px', fontWeight: 600, cursor: 'pointer' }}
                          >Lihat Detail</button>
                        </div>
                      </Popup>
                    </Marker>
          ...
          <div className="bg-white rounded-xl border border-slate-200/70 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden flex flex-col">
            <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-xs font-bold text-slate-700 uppercase tracking-wider">Daftar Prioritas Lokasi</h2>
              </div>
              <span className="text-[10px] font-medium text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">Top {priorityList.length}</span>
            </div>
            <div className="overflow-x-auto" style={{ maxHeight: '280px', overflowY: 'auto' }}>
              <table className="w-full text-left text-xs whitespace-nowrap">
                <thead className="bg-slate-50/80 text-slate-400 font-semibold sticky top-0 backdrop-blur-sm">
                  <tr>
                    <th className="px-3 py-2.5 text-[11px] uppercase tracking-wider">Nama Lokasi</th>
                    <th className="px-3 py-2.5 text-center text-[11px] uppercase tracking-wider">Skor</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100/80">
                  {priorityList.map((item, idx) => (
                    <tr key={idx} 
                      className={'hover:bg-blue-50/50 transition-all duration-200 cursor-pointer group/row '+(selLoc===item.name?'bg-blue-50/80 shadow-sm':'')}
                      onClick={()=>clickLoc(item.name)}
                    >
                      <td className="px-3 py-2.5">
                        <div className="flex items-center gap-2.5">
                          <div className={`w-2 h-2 rounded-full flex-shrink-0 ring-2 ring-white shadow-sm transition-transform group-hover/row:scale-125 ${
                            item.status === 'Merah' ? 'bg-rose-500' : 
                            item.status === 'Kuning' ? 'bg-amber-400' : 
                            'bg-emerald-500'
                          }`} />
                          ...
                          <span className="font-medium text-slate-700 truncate max-w-[140px] group-hover/row:text-blue-700 transition-colors">{item.name}</span>
                        </div>
                      </td>
                      <td className="px-3 py-2.5 text-center">
                        <div className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md font-bold text-[11px] tabular-nums ${
                          ...
                          {item.score}
                          <span className={'text-[9px] font-medium '+(item.status==='Merah'?'text-rose-400':item.status==='Kuning'?'text-amber-400':'text-emerald-400')}>
                            {item.status}
                          </span>
                        </div>
                      </td>
          ...
          <div className="group bg-white rounded-xl border border-slate-200/70 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
            ...
            <div className="relative h-44 bg-slate-200 overflow-hidden">
              {selLoc && getImageUrl(selLoc) ? (
                <img src={getImageUrl(selLoc)} alt={selLoc}
              ...
              {/* Decorative pattern */}
              <div className="absolute inset-0 opacity-[0.04]" style={{backgroundImage:'radial-gradient(circle at 50% 0%, white 1px, transparent 1px)', backgroundSize:'20px 20px'}} />
              
              <div className="absolute bottom-0 left-0 right-0 p-5">
                <div className="flex justify-between items-end w-full">
                  <div>
                    <h2 className="text-2xl font-bold text-white drop-shadow-lg tracking-tight">{selLoc || 'Pilih Lokasi'}</h2>
                    <div className="flex items-center gap-2 mt-1.5">
                      <span className="text-blue-200/80 text-xs flex items-center gap-1">
                        <MapPin size={12} /> {locDetail?.category || 'Klik lokasi di tabel prioritas atau peta'}
                      </span>
                      {locDetail && (
                        <>
                          <span className="text-blue-200/40">|</span>
                          <span className="text-blue-200/80 text-xs">{locDetail.total_reviews} review</span>
                        </>
                      )}
                    </div>
                  </div>
                  {locDetail && (
                    <div className="flex bg-black/40 backdrop-blur-md px-3 py-2 rounded-xl items-center gap-1.5 border border-white/10 shadow-lg">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <Star key={star} size={14} className={star <= Math.round(locDetail.rating || 2) ? "text-amber-300 fill-amber-300 drop-shadow-sm" : "text-white/30"} />
                      ))}
                      <span className="text-white text-xs font-bold tabular-nums ml-0.5">{(locDetail.rating || 0).toFixed(1)}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="p-5 flex flex-col gap-6">
                  ...
                  <div className="h-40 w-full relative">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        ...
                        <Tooltip contentStyle={{ borderRadius: '10px', fontSize: '12px', border: '1px solid #e2e8f0', boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }} />
                      </PieChart>
                    </ResponsiveContainer>
                  </div>
                  <div className="flex flex-wrap gap-x-3 gap-y-1.5 justify-center mt-2.5">
                    {pieData.map((item, i) => (
                      <div key={item.name} className={`flex items-center gap-1.5 text-[10px] font-medium animate-fadeIn`} style={{animationDelay:`${i*80}ms`}}>
                  ...
                  <div className="h-40 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={lineData}>
                        ...
                        <Line type="monotone" dataKey="keluhan" stroke="url(#lineGrad)" strokeWidth={2.5} dot={{ r: 2.5, fill: '#f43f5e', strokeWidth: 2, stroke: '#fff' }} activeDot={{ r: 4.5, fill: '#f43f5e', strokeWidth: 2, stroke: '#fff' }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </div>
              ...
              <div className="border-t border-slate-100 pt-5">
                <h3 className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-2">
                  <MessageSquare size={13} className="text-slate-400" /> Kutipan Ulasan Terkait
                </h3>
                <div className="flex flex-col gap-2.5 max-h-[240px] overflow-y-auto pr-1 custom-scrollbar">
                  {selLoc && recommendations.length > 0 ? (
                    getRelevantReviews().length > 0 ? (
                      ...
                      (locDetail?.recent_reviews || reviews).slice(0, 3).map((review, idx) => (
                        <ReviewCard key={idx} review={review} />
                      ))
                    )
                  ) : (
                  ...
                  {(!selLoc || recommendations.length === 0) && reviews.length === 0 && (
                    <p className="text-sm text-slate-400 italic text-center py-6">Pilih lokasi untuk melihat ulasan terkait.</p>
                  )}
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-blue-50/80 to-indigo-50/40 p-5 rounded-xl border border-blue-100/80">
                <h3 className="text-[11px] font-bold text-blue-800 uppercase tracking-wider mb-4 flex items-center gap-2">
                  <CheckCircle2 size={14} className="text-blue-600" /> Rekomendasi Tindakan
                </h3>
                {recommendations.length > 0 ? (
                  <ul className="space-y-3">
                    {recommendations.map((r, i) => (
                      <li key={i} className="flex items-start gap-3 bg-white/70 p-3.5 rounded-xl border border-blue-100/60 shadow-sm">
                        <span className={'w-6 h-6 flex-shrink-0 rounded-lg flex items-center justify-center text-xs font-bold mt-0.5 text-white shadow-sm '+(r.priority==='Kritis'?'bg-gradient-to-br from-rose-500 to-red-600':r.priority==='Tinggi'?'bg-gradient-to-br from-orange-400 to-amber-600':'bg-gradient-to-br from-blue-500 to-indigo-600')}>{i+1}</span>
                        <div className="flex-1 min-w-0">
                          ...
                          <span className={'inline-block text-[10px] font-semibold px-2 py-0.5 rounded-full mt-2 '+(r.priority==='Kritis'?'bg-rose-50 text-rose-700 ring-1 ring-rose-200/50':r.priority==='Tinggi'?'bg-amber-50 text-amber-700 ring-1 ring-amber-200/50':'bg-blue-50 text-blue-700 ring-1 ring-blue-200/50')}>{r.priority}</span>
                        </div>
                      </li>
                    ))}
                  </ul>
                  ...
                  <div className="text-center py-6">
                    ...
                    <p className="text-sm text-slate-500">Pilih lokasi untuk melihat rekomendasi.</p>
                  </div>
                )}
              </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="group bg-white rounded-xl border border-slate-200/70 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden flex flex-col h-full">
            ...
            <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-3 bg-gradient-to-r from-rose-50/50 to-amber-50/30">
              ...
              <div>
                <h2 className="text-sm font-bold text-slate-900">Notifikasi Anomali</h2>
                <p className="text-[11px] text-slate-500">Lonjakan keluhan mendadak terdeteksi</p>
              </div>
            </div>
            ...
            <div className="p-4 flex-1 flex flex-col gap-3">
              {alerts.map((alert, idx) => (
                ...
                >
                  ...
                  <div className={`absolute left-0 top-0 bottom-0 w-[3px] rounded-l-xl bg-gradient-to-b ${
                    alert.color && alert.color.includes('red') ? 'from-rose-500 to-red-400' : 
                    alert.color && alert.color.includes('orange') ? 'from-orange-400 to-amber-400' : 
                    'from-amber-400 to-yellow-400'
                  }`} />
                  <div className="pl-4">
                    <div className="flex justify-between items-start mb-2">
                      <span className="text-[10px] font-semibold text-slate-400 flex items-center gap-1.5 uppercase tracking-wider">
                        Z-Score: <span className="font-bold tabular-nums text-slate-600">{alert.z_score}</span>
                      </span>
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1
                        ${alert.status === 'Kritis' ? 'bg-rose-50 text-rose-700 ring-1 ring-rose-200/50' : 
                          alert.status === 'Tinggi' ? 'bg-amber-50 text-amber-700 ring-1 ring-amber-200/50' : 
                          'bg-orange-50 text-orange-700 ring-1 ring-orange-200/50'}
                      `}>
                        {alert.status}
                      </span>
                    </div>
                    <h4 className="text-sm font-bold text-slate-800 mb-2 group-hover/alert:text-blue-700 transition-colors">{alert.location}</h4>
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <div className="text-[9px] text-slate-400 font-medium mb-0.5">Lonjakan</div>
              ...
              {alerts.length === 0 && (
                <div className="text-center py-10">
                  ...
                  <p className="text-sm font-medium text-slate-600">Semua Lokasi Normal</p>
                  <p className="text-xs text-slate-400 mt-1">Tidak ada anomali terdeteksi</p>
                </div>
              )}
